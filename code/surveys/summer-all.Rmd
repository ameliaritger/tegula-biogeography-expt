...
```{r}
# plot scico color palette roma
library(scico)
scico(n=5, palette="roma", direction=-1)

ggplot(all_group, aes(x = factor(location, levels = rev(unique(location))), y=density_per_m2, fill=species)) +
  geom_bar(position="fill", stat="identity") + #facet_wrap(~year)
  ylab("Species proportion") +
  xlab("") +
  labs(fill="") +
  theme_bw() +
  scale_fill_manual(values=c("#023198", "#389CC6", "#7E1700", "#C0E9C2", "#B58B31")) + #colors for species
...
ggsave(here("figures", "dissertation", "forNIOZ.png"), height=5, width=7)
```
