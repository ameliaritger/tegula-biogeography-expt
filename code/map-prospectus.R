...
sites <- tribble(
  ...
  "Bodega MR", 38.318757561054554, -123.07218556931106, "#0072B2", "5",
  "Lompoc Landing", 34.7074307110824, -120.60051345263794, "#0072B2", "5",
  "Alegria", 34.46907703880511, -120.27191157990546, "#0072B2", "5",
  "Cabrillo NM", 32.66854992729292, -117.24431377177557, "#D55E00", "0"
)
...
ggplot() + 
  ...
  geom_text(data=sites, aes(x=long, y=lat, label=name), size=6.5,
            hjust=ifelse(sites$name=='Alegria',1.1, ifelse(sites$name=='Bodega MR', 1.1, 1.1)), 
            vjust=ifelse(sites$name=='Alegria',1.5, ifelse(sites$name=='Fogarty \nCreek', 0.5, ifelse(sites$name=='Sokol \nPoint', 0.75, ifelse(sites$name=='Bodega MR', 1, 0))))) +
  scale_color_manual(values = c("#D55E00", "#D55E00", "#0072B2", "#0072B2", "#D55E00","#0072B2")) +
