...
alg_no_noise <- alg_detide %>%
  mutate(p_h=ifelse((date_time > ymd_hms("2021-07-17 11:00:00") & date_time < ymd_hms("2021-07-17 19:30:00")), NA, p_h),
         temp_c=ifelse((date_time > ymd_hms("2021-07-17 11:00:00") & date_time < ymd_hms("2021-07-17 19:30:00")), NA, temp_c)) %>% #super noisy section of time for pH (not temp though) in mid July
  mutate(p_h=ifelse((date_time > ymd_hms("2021-09-09 10:15:00") & date_time < ymd_hms("2021-09-09 13:00:00")), NA, p_h),
         temp_c=ifelse((date_time > ymd_hms("2021-09-09 10:15:00") & date_time < ymd_hms("2021-09-09 13:00:00")), NA, temp_c)) %>% #super noisy section of time for pH (not temp) in early September
...
bml_hh <- bml_auto %>%
  mutate(tide_height=tide,
         diff=lag(temp_c, default = first(temp_c))-temp_c,
         im_event=ifelse(lag(temp_c, default = first(temp_c))-temp_c>=(9/4), "post-event", "null"),
         im_event_real=ifelse(lead(im_event)=="post-event", "event", "nope"))

ggplot(bml_hh, aes(x=date_time, y=temp_c)) +
  geom_point(aes(color=im_event_real), size=1) +
  geom_line(alpha=0.3)
                
# Add in wave height
bml_wave <- read_csv(here("data","sensors", "wave-bml2021.csv")) %>% 
  clean_names() %>%
  filter(wvht<99.00) %>%
  mutate(mm_2=mm_2+5) %>%
  unite("date", "number_yy", "mm", "dd", sep="/") %>%
  unite("time", "hh", "mm_2", sep=":") %>%
  unite("date_time", "date", "time", sep=" ") %>%
  mutate(date_time=ymd_hm(date_time)) %>% #apply lubridate to date and time
  select(date_time, wvht)

bml_hh_wvht <- full_join(bml_wave, bml_hh) %>%
  fill(wvht, .direction = "updown") %>% #fill in empty wave measurements with previous values before
  #distinct(date_time, .keep_all = TRUE) %>% #remove duplicated data (not sure where those came from)
  filter(!is.na(p_h)) #create df without NA values

##whoops, these wave height data are from 2020...let's check it manually?
```
...
wave_function <- function(station_code, input_month){
  ...
  wave_data <- read_delim(paste0("https://www.ndbc.noaa.gov/view_text_file.php?filename=", station_code, input_month, "2021.txt.gz&dir=data/stdmet/", month_str), delim = "\ ", col_types="n") %>%
    slice(-1) %>%
    clean_names() %>%
    select(1:5, 10) %>%
    rename(year = 1,
    ...
    mutate(min=min+4,
           hr=ifelse(min==60, hr+1, hr),
           min=ifelse(min==60, "00", min)) %>%
    unite("date", "year", "month", "day", sep="/") %>%
    unite("time", "hr", "min", sep=":") %>%
    unite("date_time", "date", "time", sep=" ") %>%
    mutate(date_time=lubridate::ymd_hm(date_time)) %>%
    mutate(station=ifelse(station_code==46214, "BML", 
                          ifelse(station_code==46011, "LOL", "ALG")))
}

#check to make sure function works
does_it_work <- wave_function(46214, 9)
```
