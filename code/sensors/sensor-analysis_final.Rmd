...
```{r}
ph_7.8 <- ph_all %>%
    filter((site == "Bodega Marine Lab" & tide == "high") | (site != "Bodega Marine Lab")) %>%  # Filter high tide for BML, keep both for others
  #filter(tide=="high") %>%
  filter(date_time > ymd_hms("2021-06-18 01:00:00"),
...
ph_prop <- ph_7.8 %>%
  group_by(site) %>%
  mutate(total_site_observations = sum(tot_observations)) %>%
  filter(threshold == "below") %>%  # Keep only "below" rows
  mutate(proportion_below_7.8 = tot_observations / total_site_observations) %>%
  select(site, proportion_below_7.8)  # Select only site and proportion

bml_7.8 <- ph_prop %>%
  filter(site == "Bodega Bay") %>%
  select(proportion_below_7.8) %>%
  pull()
...
ph_table <- tribble(
  ...
  "Bodega Marine Reserve", bml_min, bml_max, bml_median, bml_cv, bml_7.8,
...
ph_table %>%
  ...
  # Set the background to white for all columns except the 'Site' column and text color to black
  column_spec(2:6, background = "white", color = "black") %>%  # Apply white background to all columns except Site
  # Apply custom color mapping to the "Site" column (Location)
  column_spec(1, 
              background = case_when(
                ph_table$Site == "Bodega Marine Reserve" ~ "#0072B2",
                ph_table$Site == "Lompoc Landing" ~ "#009E73", 
                ph_table$Site == "Alegria" ~ "#D55E00",
                TRUE ~ "white"
              ), color = "black", bold = TRUE) %>%  # Set text color to black for Site column
  # Header row styling
  row_spec(0, bold = TRUE, color = "black", background = "white")

#html_file <- "table.html"
#save_html(html_table, file = html_file)

```


```{r}
#gtsave(here("figures", "dissertation", "table_ph_final.png"))

# Calculate min, max, average temp values for each site
alg_min <- (min(alg_table$temp_hobo_c))
alg_max <- (max(alg_table$temp_hobo_c))
alg_median <- (median(alg_table$temp_hobo_c))
alg_mean <- (mean(alg_table$temp_hobo_c))
alg_sd <- (sd(alg_table$temp_hobo_c))
alg_cv <- (alg_sd/alg_mean)*100 #coefficient of variation

lol_min <- (min(lol_table$temp_hobo_c))
lol_max <- (max(lol_table$temp_hobo_c))
lol_median <- (median(lol_table$temp_hobo_c))
lol_mean <- (mean(lol_table$temp_hobo_c))
lol_sd <- (sd(lol_table$temp_hobo_c))
lol_cv <- (lol_sd/lol_mean)*100 #coefficient of variation

bml_min <- (min(bml_table$temp_hobo_c))
bml_max <- (max(bml_table$temp_hobo_c))
bml_median <- (median(bml_table$temp_hobo_c))
bml_mean <- (mean(bml_table$temp_hobo_c))
bml_sd <- (sd(bml_table$temp_hobo_c))
bml_cv <- (bml_sd/bml_mean)*100 #coefficient of variation

temp_table <- tribble(
  ~"Site", ~"Min temp", ~"Max temp", ~"Median temp",  ~"CV", 
  "Bodega Marine Reserve", bml_min, bml_max, bml_median, bml_cv,
...
temp_table %>%
  ...
  row_spec(0, bold = TRUE) %>%  # Make header bold
  # Set the background to white for all columns except the 'Site' column and text color to black
  column_spec(2:5, background = "white", color = "black") %>%  # Apply white background to all columns except Site
  # Apply custom color mapping to the "Site" column (Location)
  column_spec(1, 
              background = case_when(
                ph_table$Site == "Bodega Marine Reserve" ~ "#0072B2",
...
#   gtsave(here("figures", "dissertation", "temp_table_final.png"))
```
