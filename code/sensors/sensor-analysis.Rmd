...
lol_temp_diff <- lol_all_1 %>%
  mutate(diff=temp_c-temp_hobo_c)

ggplot(lol_temp_diff, aes(x=date_time, y=diff)) +
  geom_line() +
  scale_x_datetime(breaks = scales::date_breaks("1 week"), 
                    labels = date_format("%m/%d %H:%m")) +
  xlab("Date time") +
...
lol_superfilter <- lol_all %>%
  filter(tide>3.232) %>%
  filter(date_time > ymd_hms("2021-10-03 23:00:00"))
         #date_time < ymd_hms("2021-09-20 23:00:00"))
...
Next steps - assess the greatest pH and temperature range in a 24 hour, 1 week period and monthly averages

CHECK line 343 to compare Hobo with Durafet temp readings, and look at the anomalies (and then also re-filter pH data based on this new temp data)
