...
filesstrings::move_files("lol-cycle-full.png", here("figures", "sensors"), overwrite=FALSE) #move the image to the "Figures" folder

```

### Try this again with code from plotly directly (https://plotly.com/r/multiple-axes/)
```{r}
fig <- plot_ly(width=2200)
...
m <- list(
  ...
  b = 30, #bottom 
  t = 30, #top
  pad = 4 #amount of padding between plot area and axis lines
)
  
fig <- fig %>% add_trace(data = lol_cycle, y=~temp_c, x = ~date_time, name = "yaxis1 data", mode = "lines", type = "scatter")
...
fig <- fig %>% add_trace(data =lol_cycle, y=~p_h, x = ~date_time, name = "yaxis2 data", yaxis = "y2", mode = "lines", type = "scatter")
...
#fig <- fig %>% add_trace(data =lol_cycle, y=~p_h, x = ~date_time, name = "yaxis3 data", yaxis = "y3", mode = "lines", type = "scatter")
...
3. Talk to someone about overfiltering vs underfiltering LOL data - look at code chunk 595 (3.232 tide) vs 530 (2.0 tide)
