...
## Remove measurements where the sensor pool was disconnected from the ocean (at a tide lower than 0, likely higher (TBD))
```{r}
lol_detide <- lol_all %>%
  filter(tide>0.5)
...
ggsave(here("figures", "sensors", "lol_detide_tide.png"), height=20, width=40, units="cm")
```
...
ggplot(lol_check, aes(x=date_time)) +
  geom_line(aes(y=tide), color="red") + 
  geom_line(aes(y=temp_c), color="blue") +
  scale_x_datetime(breaks = scales::date_breaks("1 week"), 
                    labels = date_format("%m/%d %H:%m")) +
...
The peak is where the tide starts coming back in (when the pool is re-connected to the ocean)

### ID where the peaks are in the data, so I can filter
```{r}
