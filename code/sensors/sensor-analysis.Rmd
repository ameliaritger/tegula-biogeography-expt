...
## Read in the data
```{r}
all <- read_csv(here("data","sensors", "sensor-data_all_zoe.csv")) %>% 
  clean_names() %>%
...
#          date=format(date_time, '%m/%d/%Y'), #create only date column
#          time=format(date_time, '%H:%M:%S')) %>% #create only time column
#   mutate(site=replace(site, site=="LOL", "Lompoc Landing"), #rename locations
#            site=replace(site, site=="ALG", "Alegria"),
#            site=replace(site, site=="BML", "Bodega Bay")) %>%
#   select(site, sensor_number, download_date, date_time, date, time, temp_c, p_h)
# 
# zoe_sum <- zoe %>%
#   group_by(site, sensor_number) %>%
#   summarize(n=n())
# 
# all_sum <- all %>%
#   group_by(site, sensor_number) %>%
...
bml_superfilter <- bml_detide %>%
  filter(date_time > ymd_hms("2021-10-01 01:00:00"),
         date_time < ymd_hms("2021-10-20 23:00:00"))  #7-30 to 8-06 is also interesting
...
#         axis.title.y=element_text(size=15))
# 
# ggsave(here("figures",  "bml_detide_tide-temp-pH.png"), height=20, width=40, units="cm")
```
...
lol_detide <- lol_all %>%
  mutate(tide_height=tide,
         tide=ifelse(tide_height>0.5, "high", "low"))
  #filter(tide=="high")
...
ggsave(here("figures", "sensors", "ph_all_detide.png"), height=20, width=40, units="cm")
...
write.csv(ph_all_detide, here("data", "sensors", "ph_detide.csv"), row.names = FALSE)
```
